...
<div class="row align-items-center vh-100 w-100 justify-content-end shadow-lg rounded m-0" id="form-auth-container">
    <div class="col-12 col-lg-4 bg-white py-5 p-lg-4 h-100" id="login-container">
        ...
        <h1 class="bg-primario text-white fw-bold fs-3 py-3 text-center">Iniciar Sesion<br>Biblioteca</h1>
        <h6 class="text-danger fs-6 text-center">Mensaje Temporal: Credenciales de Prueba son biblioteca@gmail.com y la contraseña es password</h6>
        <form action="{{ route('authenticatebiblioteca') }}" method="POST">
            @csrf
            @method('POST')
            <div class="d-flex flex-column">
                <label class="fs-5 text-center fw-bold my-2" for="email">Correo Electronico:</label>
                <input type="email" class="form-control" id="email" name="email" placeholder="Correo electronico" data-index="1" autocomplete="off">
            </div>
