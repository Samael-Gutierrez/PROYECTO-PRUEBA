...
<div class="container-fluid p-0">
    ...
    <div class="col-12 bg-primario rounded-top mb-3">
        <div class="col-12 py-4">
            <h1 class="text-white fw-bold text-uppercase text-center fs-4 mb-2">Ciclos Escolares</h1>
    ...
    <div class="row w-100 h-100 m-0 p-0">
        ...
        <!-- Header Section -->
        <form action="{{ route('ciclos.index') }}" method="POST" class="row w-100 p-0 mx-auto align-items-center" id="form-number-items">
            <div class="col-12 col-md-4 text-center my-2">
                @csrf
                @method('GET')
                <select name="paginate" id="select-number-items">
                    <option value="10" @if($ciclos->count() == '10') selected @endif >10</option>
                    <option value="50" @if($ciclos->count() == '50') selected @endif >50</option>
                    <option value="100" @if($ciclos->count() == '100') selected @endif >100</option>
                    <option value="250" @if($ciclos->count() == '250') selected @endif >250</option>
                </select>
            </div>
            ...
            <div class="col-12 col-md-4">
                <a href="{{ route('ciclos.create') }}" class="btn btn-primario text-white btn-sm d-block mx-auto w-75">Crear Nuevo</a>
            </div>
        </form>
        <!-- Header Section -->
                ...
                <table class="table table-primaria d-none d-lg-table text-center align-center">
                    ...
                    <tbody id="">
                        ...
                        <tr class="d-none d-lg-table-row col-12">
                            <td class="col-4">{{$ciclo->nombre}}</td>
                            <td class="col-5">
                                <span @class([ 'badge bg-success'=> $ciclo->activo,
                                    'badge bg-danger' => !$ciclo->activo ])>
                              ...
                              {{--  start botón de editar --}}
                                <div class="row justify-content-evenly w-100">
                                    <div class="col text-center px-0">
                                        <a href="{{ route('ciclos.edit', $ciclo) }}" class="btn btn-sm btn-primario text-white w-20" >
                                    ...
                                    <div class="col text-center px-0">
                                        <form action="{{ route('ciclos.toggle.status', $ciclo) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            ...
                                            <button type="submit" @if($ciclo->activo)
                                                value="Desactivar"
                                                @else
                                                value="Activar"
                                                @endif
                                                @class([
                                                'btn-secundario text-white w-20 btn btn-sm' => $ciclo->activo,
                                                'btn-success w-20 btn btn-sm' => !$ciclo->activo
                                                ])
        ...
        <div class="card shadow-lg my-3 py-3 mx-auto d-lg-none">
            <div class="card-title text-center fw-bold">{{ $ciclo->nombre }}</div>
            ...
            <div class="card-footer">
                <div class="row">
                    <div class="col text-center">
                        <a href="{{ route('ciclos.edit', $ciclo) }}" class="btn btn-sm btn-primario text-white">Editar</a>
                    </div>
                    <div class="col text-center">
                        <form action="{{ route('ciclos.toggle.status', $ciclo) }}" method="POST">
                            @csrf
    ...
    <div class="row my-3 w-100 h-100 p-0">
        <div class="col-12 col-md-3">
            <p class="text-secondary text-center fst-italic">Mostrando {{ $ciclos->count() }} resultados de {{ $ciclos->total() }}</p>
        </div>
